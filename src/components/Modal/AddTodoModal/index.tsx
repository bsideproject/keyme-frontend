...
import { useCreateTodo } from "~hooks/queries/todo";
...
const Container = styled.div<{ anim: boolean }>`
  ...
  height: 100%;
  z-index: 10000;
  animation: ${(props) => (props.anim === false ? rtlanimation : ltranimation)} 0.3s ease-in-out;
...
const AddTodoModal: FC<ModalProps> = ({ closeModal }) => {
  const [todoInput, onChangeTodoInput, resetTodo] = useInput("");
  const [isOpen, setIsOpen] = useState(true);
  const { createTodoMutate, isLoading } = useCreateTodo();

  const [selectedKeyResult, setSelectKeyResult] = useState("건강");

  const addTodoSubmit = useCallback(() => {
    createTodoMutate({ title: todoInput });
  }, [todoInput]);
        ...
        <div className="key-result-wrapper">
          ...
          <div className="key-result-box">
            ...
            <div className="key-result-lists">
              <RadioGroup
                selectedValue={selectedKeyResult}
                onChange={(e) => {
